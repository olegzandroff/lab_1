template <typename T>
void interface_sem_3_tasks_menu_item_2(const T* array, int length)
{
	std::cout << "{item_2}\n";
	...
	const int absmax_i_n = sem_3_absmax_i_n(array, length);
	const int absmin_i_n = sem_3_absmin_i_n(array, length);
	const int* absmax_i = sem_3_absmax_i(array, length);
	const int* absmin_i = sem_3_absmin_i(array, length);
	...
	if (sem_3_bad_scenario_1(array, length))
	{
		...
		int* absmax_i = sem_3_absmax_i(array, length);
		int* absmin_i = sem_3_absmin_i(array, length);
